...
class TestAds:
    ...
    def test_create_ad_authorized_user(self, driver, wait, fin2):
        ad_name = generate_unique_ad_name()
        print(ad_name)
        driver.get(BASE_URL)
        wait.until(ec.element_to_be_clickable(locators.LOGIN)).click()
        wait.until(ec.visibility_of_element_located(locators.EMAIL)).send_keys(fin2.login)
        driver.find_element(*locators.PASSWORD).send_keys(fin2.password)
        ...
        assert driver.find_element(*locators.MY_ADS_FIRST_TITLE).text == ad_name